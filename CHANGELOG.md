...
## v0.2.1 · 2026-07-09 — 入口可测性与核心契约

- 修复 `server.py` 在测试导入时误解析 `unittest discover` 参数的问题；端口只在直接启动服务时解析。
- 为端口解析补充回归测试，保护 API 测试入口。
- 新增 `docs/project-core-contract.md`，明确数据权威产物、只读审计、来源可追踪和医疗边界。
- README 增加维护核心与最小验收命令，方便后续继续演化而不偏离项目出发点。
